...
// one writer per file at a time, whichever FileStore instance holds it
...
export class FileStore {
  ...
  /**
   * Load, mutate in place, save — serialized per file (not re-entrant) so concurrent writers
   * cannot overwrite each other's rows. Resolves to whatever mutate returns.
   */
  async update(key, mutate) {
    const previous = writeQueues.get(this.basename) ?? Promise.resolve()
    let release
    writeQueues.set(
      this.basename,
      new Promise((resolve) => {
