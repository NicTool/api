...
export default class User {
  constructor(args) {
    ...
    this.cfg = new Config().getSync('session')
  ...
  async authenticate(authTry) {
    ...
    for (const u of await this.mysql.execute(query, [username, groupName])) {
      if (
  ...
  async create(args) {
    ...
    const userId = await this.mysql.insert(
      `INSERT INTO nt_user`,
  ...
  async destroy(args) {
    const u = await this.getAdmin(args)
    if (u.length === 1) {
      await this.mysql.execute(`DELETE FROM nt_user WHERE nt_user_id=?`, [u[0].id])
    }
  }
