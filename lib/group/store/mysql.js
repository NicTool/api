...
class Group extends GroupBase {
  ...
  async addToSubgroups(gid, parent_gid, rank = 1000, db = Mysql) {
    ...
    await db.execute('INSERT INTO nt_group_subgroups (nt_group_id,nt_subgroup_id,`rank`) VALUES(?,?,?)', [
      parent_gid,
      gid,
      rank,
    ])
    ...
    const parent = await db.execute(
      'SELECT parent_group_id AS parent_gid FROM nt_group WHERE nt_group_id = ?',
      [parent_gid],
    )
    if (parent.length === 1 && parent[0].parent_gid !== 0) {
      // both the insert and the parent walk must use the same connection:
      // reads through the shared connection cannot see this transaction's
      // uncommitted writes and would resurrect stale ancestor rows
      await this.addToSubgroups(gid, parent[0].parent_gid, rank - 1, db)
    }
  }
  ...
  async put(args) {
    ...
    const update = () => Mysql.update(`nt_group`, `nt_group_id=${id}`, mapToDbColumn(args, groupDbMap))

    if (args.parent_gid === undefined) {
      const r = await Mysql.execute(...update())
      return r.changedRows === 1
    }

    return Mysql.transaction(async (tx) => {
      const r = await tx.execute(...update())
      await this.rebuildSubgroups(id, tx)
      return r.changedRows === 1
    })
  }

  async rebuildSubgroups(rootGid, conn = Mysql) {
    const db = conn.execute ? conn : Mysql
    const groups = await db.execute(
      `WITH RECURSIVE descendants AS (
         SELECT nt_group_id AS id, parent_group_id AS parent_gid
         FROM nt_group WHERE nt_group_id = ?
         UNION ALL
         SELECT g.nt_group_id, g.parent_group_id
         FROM nt_group g
         JOIN descendants d ON g.parent_group_id = d.id
       )
       SELECT id, parent_gid FROM descendants`,
      [rootGid],
    )
    ...
    await db.execute(
      `DELETE FROM nt_group_subgroups WHERE nt_subgroup_id IN (${placeholders})`,
      groups.map((group) => group.id),
    )
    for (const group of groups) {
      await this.addToSubgroups(group.id, group.parent_gid, 1000, db)
    }
  }
  ...
  async destroy(args) {
    ...
    await Mysql.execute(
      `DELETE FROM nt_perm WHERE nt_group_id = ? AND (nt_user_id IS NULL OR nt_user_id = 0)`,
      [args.id],
    )
