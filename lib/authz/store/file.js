...
class AuthzRepoFile extends AuthzBase {
  ...
  async liveSessionGroup(userId, sessionId, oldestSec) {
    ...
    // file stores keep no last_access per session; presence is the best
    // available signal that the session is still live
    const session = sessions.find(
      (s) => s.user_id === userId && s.id === sessionId && (s.last_access ?? Infinity) >= oldestSec,
    )
  ...
  async permissionRecord(permissionId) {
    ...
    for (const u of users) {
      for (const p of u.permissions ?? []) {
        if (p.permissionId === permissionId || p.id === permissionId) {
          return { uid: u.id, gid: null, target_gid: u.gid }
        }
      }
    }
    for (const g of groups) {
