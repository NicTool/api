...
const CREATE_REQUIRES_GROUP = new Set(['group', 'nameserver', 'user', 'zone', 'zonerecord'])
...
class AuthzBase {
  async checkPermission(credentials, resource, action, objectId, opts) {
    ...
    if (action === 'create') {
      ...
      if (targetGid === undefined || targetGid === null) {
        ...
        const inTree = await this.isInGroupTree(credentials.group.id, targetGid)
        if (!inTree) {
          if (resource === 'zonerecord' && opts?.targetZoneId) {
            const delegation = await this.getDelegateAccess(credentials.group.id, opts.targetZoneId, 'zone')
          ...
          return deny(
            `No Access Allowed to that object` + ` (${DELEGATE_TYPE[resource] ?? 'GROUP'} : ${targetGid})`,
          )
        }
      }
      return allow()
    ...
    if (resource === 'nameserver' && action === 'read' && (await this.isActiveObject(resource, objectId))) {
      return allow()
    }
    ...
    const delegation = await this.getDelegateAccess(credentials.group.id, objectId, resource)
    if (delegation) {
      ...
      const permField = action === 'deleteDelegation' ? 'perm_delete' : `perm_${action}`
  ...
  canTransitionPermissions(userPerm, before, after) {
    ...
    if (userPerm.user?.write !== true && Boolean(before?.self_write) !== Boolean(after?.self_write)) {
      return false
    }
  ...
  async getCurrentCredentials(credentials) {
    ...
    const gid = await this.liveSessionGroup(credentials.user.id, credentials.session.id, oldest)
