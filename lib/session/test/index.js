...
describe('session', function () {
  ...
  describe('create', () => {
    ...
    it('creates a distinct row for each login', async () => {
      const secondId = await Session.create({
        nt_user_id: sessionUser.id,
        session: '3.0.0',
        last_access: parseInt(Date.now() / 1000, 10),
      })
      assert.notEqual(secondId, sessionId)
      await Session.delete({ id: secondId })
      assert.ok(await Session.get({ id: sessionId }))
    })
  })
