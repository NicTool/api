...
async function withUserWriteLock(fn) {
  ...
  let release
  userWriteQueue = new Promise((resolve) => {
    release = resolve
  })
  await previous
  try {
...
class UserRepoFile extends UserBase {
  ...
  async authenticate(authTry) {
    ...
    const groups = await this.groupFile.load('group')

    const users = await this._load()
    for (const u of users) {
      if (u.username !== username) continue
      if (u.deleted) continue
      const group = groups.find((g) => g.id === u.gid && g.name === groupName && !g.deleted)
      if (!group) continue
      ...
      if (valid) {
        ...
        const g = { id: result.gid, name: group.name }
        delete result.gid
  ...
  async get(args) {
    ...
    users = users.map((u) => ({ ...u, group_name: groupNames.get(u.gid) ?? '' }))
    ...
    const sortBy = ['id', 'username', 'email', 'first_name', 'last_name', 'group_name'].includes(args.sort_by)
      ? args.sort_by
      : 'username'
