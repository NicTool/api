...
describe('user', function () {
  ...
  // these drive the store through raw SQL, so they only apply to the MySQL repo.
  // Left running under TOML they hang: the pool never connects and its open
  // handle keeps the test process alive forever.
  describe('password upgrade on login', { skip: storeType !== 'mysql' }, () => {
    ...
    async function getDbPassword() {
      const rows = await Mysql.execute('SELECT password, pass_salt FROM nt_user WHERE nt_user_id = ?', [
        upgradeUserId,
      ])
      return rows[0]
    }
    ...
    async function cleanup() {
      await Mysql.execute('DELETE FROM nt_user WHERE nt_user_id = ?', [upgradeUserId])
    }
