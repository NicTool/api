...
class Config {
  ...
  async get(name) {
    ...
    const str = await fs.readFile(path.join(confDir, `${name}.toml`), 'utf8')
    const cfg = parse(str)
    applyEnvOverrides(name, cfg)
    // if (this.debug) console.debug(cfg)

    if (name === 'http') {
      const tls = await loadPEM(confDir)
      if (tls) cfg.tls = tls
    }
  ...
  getSync(name) {
    ...
    const str = fsSync.readFileSync(path.join(confDir, `${name}.toml`), 'utf8')
    const cfg = parse(str)
    applyEnvOverrides(name, cfg)
    // if (this.debug) console.debug(cfg)

    if (name === 'http') {
      const tls = loadPEMSync(confDir)
      if (tls) cfg.tls = tls
    }
...
// Published tarballs carry no conf.d — it held live secrets, so it is no longer
// in package.json#files. Seed it on first run instead, minting the secrets here
// so that every install does not share a signing key baked into the package.
function seedConfDir() {
  if (fsSync.existsSync(confDir)) return

  fsSync.mkdirSync(confDir, { recursive: true, mode: 0o700 })
  fsSync.writeFileSync(path.join(confDir, 'http.toml'), httpSeed(), { mode: 0o600 })
  console.log(`seeded ${path.resolve(confDir, 'http.toml')} with generated secrets`)
}

function httpSeed() {
  const secret = (bytes) => crypto.randomBytes(bytes).toString('hex')

  // @hapi/cookie requires a password of 32 characters or more.
