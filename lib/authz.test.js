...
describe('checkPermission', () => {
  describe('create actions', () => {
    it('allows create when user has permission', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'create', undefined, { targetGroupId: 4200 })
      assert.equal(r.allowed, true)
    })

    it('allows create into child group', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'create', undefined, { targetGroupId: 4201 })
      assert.equal(r.allowed, true)
    })

    it('denies create when user lacks permission', async () => {
      const r = await Authz.checkPermission(credsLimited, 'zone', 'create', undefined)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /Not allowed to create/)
    })

    it('denies create when target group not in tree', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'create', undefined, { targetGroupId: 4202 })
      assert.equal(r.allowed, false)
  ...
  describe('self-user restrictions', () => {
    it('denies delete self', async () => {
      const r = await Authz.checkPermission(credsFull, 'user', 'delete', 4200)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /Not allowed to delete self/)
    })

    it('allows write self when self_write=true', async () => {
      const r = await Authz.checkPermission(credsFull, 'user', 'write', 4200)
      assert.equal(r.allowed, true)
    })

    it('denies write self when self_write=false', async () => {
      const r = await Authz.checkPermission(credsNoself, 'user', 'write', 4202)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /Not allowed to modify self/)
    })

    it('allows read self', async () => {
      const r = await Authz.checkPermission(credsFull, 'user', 'read', 4200)
      assert.equal(r.allowed, true)
    })
  })
  ...
  describe('own-group restrictions', () => {
    it('denies write to own group', async () => {
      const r = await Authz.checkPermission(credsFull, 'group', 'write', 4200)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /Not allowed to edit your own group/)
    })

    it('denies delete own group', async () => {
      const r = await Authz.checkPermission(credsFull, 'group', 'delete', 4200)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /Not allowed to delete your own group/)
    })
  })

  describe('nameserver reads', () => {
    it('allows authenticated read of an active nameserver', async () => {
      const r = await Authz.checkPermission(credsLimited, 'nameserver', 'read', 4200)
      assert.equal(r.allowed, true)
    })
  })
  ...
  describe('group tree ownership', () => {
    it('allows read of in-tree zone', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'read', 4200)
      assert.equal(r.allowed, true)
    })

    it('allows write of in-tree zone with permission', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'write', 4200)
      assert.equal(r.allowed, true)
    })

    it('denies write when user lacks action permission', async () => {
      const r = await Authz.checkPermission(credsLimited, 'zone', 'write', 4201)
      assert.equal(r.allowed, false)
    })
  })
  ...
  describe('delegation access', () => {
    it('allows read of delegated zone', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'read', 4201)
      assert.equal(r.allowed, true)
    })

    it('allows write of delegated zone when perm_write=1', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'write', 4201)
      assert.equal(r.allowed, true)
    })

    it('denies delete of delegated zone when perm_delete=0', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'delete', 4201)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /no 'delete' permission/)
    })

    it('perm_delete never permits deleting the delegated object', async () => {
    ...
    it('allows delegate action when perm_delegate=1', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'delegate', 4201)
      assert.equal(r.allowed, true)
    })
  })
  ...
  describe('pseudo-delegation (zone record via parent zone)', () => {
    it('allows read of zone record in delegated zone', async () => {
      const r = await Authz.checkPermission(credsFull, 'zonerecord', 'read', 4201)
      assert.equal(r.allowed, true)
    })
  })
  ...
  describe('direct zone record delegation', () => {
    it('allows read of directly delegated zone record', async () => {
      const r = await Authz.checkPermission(credsFull, 'zonerecord', 'read', 4202)
      assert.equal(r.allowed, true)
    })

    it('denies delete when perm_delete=0', async () => {
      const r = await Authz.checkPermission(credsFull, 'zonerecord', 'delete', 4202)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /no 'delete' permission/)
    })
  })
  ...
  describe('deny fallthrough', () => {
    it('denies access to object not in tree and not delegated', async () => {
      const r = await Authz.checkPermission(credsLimited, 'zone', 'read', 4200)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /No Access Allowed/)
    })

    it('denies when object does not exist', async () => {
      const r = await Authz.checkPermission(credsFull, 'zone', 'read', 99999)
      assert.equal(r.allowed, false)
      assert.match(r.msg, /No Access Allowed/)
    })
  })
...
describe('capPermissions', () => {
  ...
  it('preserves usable nameservers the caller cannot manage', () => {
    const capped = Authz.capPermissions(
      { nameserver: { usable: ['1'] } },
      { usable_ns: [1] },
      { nameserver: { usable: ['1', '2'] } },
    )
    assert.deepEqual(capped.usable_ns, ['1', '2'])
  })

  it('rejects an inherited-permission transition the caller cannot grant', () => {
    const userPerm = { zone: { delete: false }, user: { write: true } }
    const before = { zone: { delete: false } }
    const after = { zone: { delete: true } }
    assert.equal(Authz.canTransitionPermissions(userPerm, before, after), false)
    assert.equal(Authz.canTransitionPermissions(userPerm, after, after), true)
  })

  it('carries unmanaged permissions into a new explicit row', () => {
    const preserved = Authz.preserveUnmanagedPermissions(
      { zone: { delete: false }, user: { write: true } },
      { name: 'explicit' },
      { zone: { delete: true } },
    )
    assert.equal(preserved.zone_delete, true)
  })
})
