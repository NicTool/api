...
class Mysql {
  ...
  // The shared connection cannot hold a transaction: concurrent requests
  // would interleave statements inside it. Transactions run on their own
  // connection and are handed an object with execute/insert/update helpers.
  async transaction(fn) {
    ...
    try {
      ...
      const tx = {
        execute: async (query, paramsArray) => {
          const [rows] = await conn.execute(query, paramsArray ?? [])
          if (/^(REPLACE|INSERT) INTO/.test(query)) return rows.insertId
          return rows
        },
        insert: (table, params) => this.insert(table, params),
        update: (table, where, params) => this.update(table, where, params),
      }
