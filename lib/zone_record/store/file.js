...
class ZoneRecordRepoFile extends ZoneRecordBase {
  ...
  async create(args, options) {
    ...
    return this.file.mutate('zone_record', (records, data) => {
      if (args.id !== undefined && records.some((record) => record.id === args.id)) {
        return idConflict('zone record', args.id, options)
      }

      if (args.id === undefined) args.id = nextId(records, data.last_id)
