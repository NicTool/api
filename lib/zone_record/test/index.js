...
describe('zone_record', function () {
  ...
  it('changes record type without retaining stale rdata fields', async () => {
    const id = 60002
    ...
    try {
      await ZoneRecord.create({
        id,
        zid: 4096,
        owner: 'type-change.example.com.',
        ttl: 300,
        type: 'A',
        address: '192.0.2.1',
      })
      assert.ok(await ZoneRecord.put({
        id,
        type: 'MX',
      ...
      assert.equal(updated.type, 'MX')
      assert.equal(updated.exchange, 'mail.example.com.')
      assert.equal(updated.preference, 10)
      assert.equal(updated.other, undefined)
    } finally {
      await ZoneRecord.destroy({ id })
    }
  })
