...
async function withZoneWriteLock(fn) {
  ...
  let release
  zoneWriteQueue = new Promise((resolve) => {
    release = resolve
  })
  await previous
  try {
...
function assertZoneNameAvailable(zones, zone, excludeId) {
  ...
  if (
    zones.some(
      (row) => row.id !== excludeId && row.deleted !== true && canonicalZoneName(row.zone) === canonical,
    )
  ) {
...
class ZoneRepoFile extends ZoneBase {
  constructor(args = {}) {
    ...
    this.nsFile = new FileStore('zone_nameserver')
  }

  async _loadNs() {
    return this.nsFile.load('zone_nameserver')
  }

  // written after the zone file, under the same lock; a crash between the
  // two leaves them out of step
  async _replaceNs(zid, ids) {
    const rows = (await this._loadNs()).filter((m) => m.zid !== zid)
    for (const nid of new Set(ids.map(Number))) rows.push({ zid, nid })
    await this.nsFile.save('zone_nameserver', rows)
  }
  ...
  async create(args) {
    ...
    return withZoneWriteLock(async () => {
      ...
      if (zone.id && zones.some((z) => z.id === zone.id)) return zone.id
      if (zone.deleted !== true) assertZoneNameAvailable(zones, zone.zone)
      if (!zone.id) zone.id = zones.reduce((max, z) => Math.max(max, z.id ?? 0), 0) + 1
      zones.push(JSON.parse(JSON.stringify(zone)))
      await this._save(zones)
      if (Array.isArray(nameservers)) await this._replaceNs(zone.id, nameservers)
      return zone.id
    })
  }
  ...
  async put(args) {
    ...
    const { nameservers, ...fields } = args
    return withZoneWriteLock(async () => {
      const zones = await this._load()
      const idx = zones.findIndex((z) => z.id === fields.id)
      if (idx === -1) return false
      if (fields.deleted === false) assertZoneNameAvailable(zones, zones[idx].zone, fields.id)

      zones[idx] = { ...zones[idx], ...fields }
      await this._save(zones)
      if (Array.isArray(nameservers)) await this._replaceNs(fields.id, nameservers)
      return true
    })
  }
  ...
  async destroy(args) {
    return withZoneWriteLock(async () => {
      ...
      await this._replaceNs(args.id, [])
      return true
    })
  }

  async nameserverIds(zid) {
    return (await this._loadNs())
      .filter((m) => m.zid === zid)
      .map((m) => m.nid)
      .sort((a, b) => a - b)
  }

  async setNameservers(zid, ids) {
    return withZoneWriteLock(async () => {
      await this._replaceNs(zid, ids)
      return true
    })
  }
