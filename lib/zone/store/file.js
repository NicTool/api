...
class ZoneRepoFile extends ZoneBase {
  ...
  async create(args, options) {
    ...
    return this.file.mutate('zone', (zones, data) => {
      if (args.id !== undefined && zones.some((zone) => zone.id === args.id)) {
        return idConflict('zone', args.id, options)
      }

      if (args.id === undefined) args.id = nextId(zones, data.last_id)
