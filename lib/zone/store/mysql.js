...
function zoneLockName(zone) {
  return `nictool-zone-${createHash('sha256').update(canonicalZoneName(zone)).digest('hex')}`
}
...
class ZoneRepoMySQL extends ZoneBase {
  ...
  async create(args) {
    ...
    return withZoneNameLock(args.zone, async (tx) => {
      if (args.deleted !== true) await assertZoneNameAvailable(tx, args.zone)
      return tx.execute(...tx.insert(`nt_zone`, mapToDbColumn(args, zoneDbMap)))
    })
  }
  ...
  async get(args) {
    ...
    ;[finalQuery, finalParams] = applyAccessScope(
      finalQuery, finalParams, gidScope, accessibleIds,
    )
  ...
  async put(args) {
    ...
    if (args.deleted === false) {
      const rows = await Mysql.execute(
        'SELECT zone FROM nt_zone WHERE nt_zone_id = ? LIMIT 1',
        [id],
      )
