...
class DelegationRepoFile extends DelegationBase {
  ...
  async _update(mutate) {
    return this.file.update('delegation', mutate)
  }
  ...
  async create(args) {
    ...
    const created = await this._update((rows) => {
      if (rows.some((r) => r.gid === gid && r.oid === oid && r.type === type)) return false
      rows.push(row)
      return true
    })
  ...
  async getDelegated(gid, type) {
    if (!TYPES[type]) return []
    const rows = (await this._load()).filter((r) => r.gid === gid && r.type === type)
    const active = []
  ...
  async getDelegates(oid, type, gid) {
    if (!TYPES[type]) return []
    let rows = (await this._load()).filter((r) => r.oid === oid && r.type === type)
  ...
  async put(args) {
    ...
    const row = await this._update((rows) => {
      const found = rows.find((r) => r.gid === gid && r.oid === oid && r.type === type)
  ...
  async delete(args) {
    const { gid, oid, type } = args
    const row = await this._update((rows) => {
      const at = rows.findIndex((r) => r.gid === gid && r.oid === oid && r.type === type)
      return at === -1 ? null : rows.splice(at, 1)[0]
    })
    if (!row) return null
  ...
  async writeLog(data, action) {
    await this.logFile.update('delegate_log', (logs) =>
      logs.push({
