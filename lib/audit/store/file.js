...
class AuditRepoFile extends AuditBase {
  ...
  async insertZoneLog(detail) {
    const rows = await this.zoneLog.load('zone_log')
    const row = { id: nextId(rows), ...detail }
    rows.push(row)
    await this.zoneLog.save('zone_log', rows)
    return row.id
  }

  async insertZoneRecordLog(detail) {
    const rows = await this.recordLog.load('record_log')
    const row = { id: nextId(rows), ...detail }
    rows.push(row)
    await this.recordLog.save('record_log', rows)
    return row.id
  }

  async insertGlobalLog(entry) {
    const rows = await this.globalLog.load('global_log')
    rows.push({
      id: nextId(rows),
      uid: entry.uid,
    ...
    await this.globalLog.save('global_log', rows)
  }
  ...
  async listGlobal(args) {
    ...
    return page(rows, args, {
      ...
      sortMap: {
        timestamp: 'timestamp', user: 'user', action: 'action', object: 'object',
        title: 'title', description: 'description', group_name: 'group_name',
      },
    })
  }
  ...
  async listZones(args) {
    ...
    return page(scoped, args, {
      ...
      sortMap: {
        timestamp: 'timestamp', user: 'user', action: 'action', zone: 'zone',
        ttl: 'ttl', description: 'description', group_name: 'group_name',
      },
    })
  }
  ...
  async listZoneRecords(args) {
    ...
    return page(rows, args, {
      ...
      sortMap: {
        ...
        type: 'type', address: 'address', ttl: 'ttl', weight: 'weight',
