...
const seen = { id: 4301, zid: zone.id, owner: 'seen.audit.example.com.', type: 'A', address: '192.0.2.1', ttl: 3600 }
const hidden = { id: 4302, zid: zone.id, owner: 'hidden.audit.example.com.', type: 'A', address: '192.0.2.2', ttl: 3600 }

before(async () => {
  await Group.create(groupCase)
  await User.create(userCase)
  await Zone.destroy({ id: zone.id })
  await Zone.create({ ...zone, mailaddr: 'hostmaster.audit.example.com.', serial: 1, refresh: 1, retry: 1, expire: 1, minimum: 1, ttl: 1 })
...
describe('audit', () => {
  ...
  it('limits record log rows to the ids a caller may read', async () => {
    ...
    assert.deepEqual(rows.map((r) => r.zrid), [seen.id])
  })
