  ...
  if (resource === 'zonerecord') {
    ...
    if (zid) {
      const rows = await Mysql.execute(
        'SELECT nt_group_id FROM nt_zone WHERE nt_zone_id = ? AND deleted = 0',
        [zid],
      )
      if (rows.length > 0) {
        return { gid: rows[0].nt_group_id, zid: Number(zid) }
      }
    }
    return null
  }
...
async function changesTarget(request, resource, action, objectId) {
  if (resource !== 'zonerecord' || action !== 'write') return true
  const rows = await Mysql.execute(
    'SELECT nt_zone_id AS zid FROM nt_zone_record WHERE nt_zone_record_id = ?',
    [objectId],
  )
  return rows.length === 0 || rows[0].zid !== Number(request.payload.zid)
}
