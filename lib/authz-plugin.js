  ...
  register(server) {
    server.ext('onPreHandler', async (request, h) => {
      ...
      if (request.auth.isAuthenticated && !isLogin) {
        ...
        // idle expiry must measure activity; Session.put throttles to once a minute
        Session.put({ id: credentials.session.id, last_access: true }).catch((err) => {
          console.error(`session ${credentials.session.id} activity: ${err.message}`)
        })
      }
        ...
        const result =
          action === 'create'
            ...
            : await Authz.checkPermissionRecord(
                credentials,
                action,
                Number(resolveId(request, permCfg.idFrom)),
              )
        return respond(result, h)
      }

      if (action === 'readDelegation') {
        const type = request.query?.type ?? 'ZONE'
        ...
        if (request.query.oid !== undefined) {
          const object = await Authz.checkPermission(
            ...
            delegatedResource,
            'read',
            Number(request.query.oid),
          )
          if (!object.allowed) return respond(object, h)
        }
        if (request.query.gid !== undefined) {
          const group = await Authz.checkPermission(credentials, 'group', 'read', Number(request.query.gid))
      ...
      if (permCfg.targetGroupFrom) {
        ...
        // a PUT that keeps the current group is an edit, not a move
        const currentGid = targetGid === undefined ? null : await Authz.getObjectGroupId(resource, objectId)
        if (targetGid !== undefined && Number(targetGid) !== currentGid) {
          if (
            resource === 'user' &&
            action === 'write' &&
          ...
          if (
            resource === 'group' &&
            action === 'write' &&
            (Number(targetGid) === objectId || (await Authz.isInGroupTree(objectId, Number(targetGid))))
          ) {
          ...
          const target = await Authz.checkPermission(credentials, 'group', 'read', Number(targetGid))
          if (!target.allowed) return respond(target, h)

          // moves stay inside the owning tree, as in v2
          if (currentGid !== null && !(await Authz.isInGroupTree(credentials.group.id, currentGid))) {
            return respond({ allowed: false, code: 404, msg: `Cannot move a delegated ${resource}` }, h)
          }
        }
      }
      ...
      if (action === 'create') {
        ...
        const target = await resolveTargetGroup(request, resource)
        opts = {
      ...
      const result = await Authz.checkPermission(credentials, resource, action, objectId, opts)
